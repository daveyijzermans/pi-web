...
func (d *queueDrainer) drainSession(sessionID string) {
	...
	// A turn can also be running outside the in-process worker (detached
	// worker-holder or terminal pi); dispatching then steers the queued item
	// into that turn. hasActiveTerminalTurn is checked separately because the
	// orphaned-web-turn override reports idle to unblock user sends — the
	// autonomous drainer must wait instead.
